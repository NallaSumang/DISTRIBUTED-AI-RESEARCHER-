...
export default function Home() {
  ...
  /* ------------------------------------------------------------------ */
  /*  SIDEBAR CONTENT (shared between desktop pane & mobile drawer)      */
  /* ------------------------------------------------------------------ */
  const SidebarContent = () => (
    <>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {history.length === 0 && (
          <p className="text-[11px] text-zinc-700 text-center mt-8 tracking-widest">
            No research yet
          </p>
        )}
        {history.map((item) => (
          <motion.button
            ...
            key={item.id}
            onClick={() => {
              setReport(stripThinking(item.report));
              setSidebarOpen(false);
              window.scrollTo({ top: 0, behavior: "smooth" });
            }}
            className="w-full text-left p-4 rounded-xl bg-white/[0.01] border border-white/[0.02] hover:bg-white/[0.03] hover:border-red-900/20 transition-all duration-500 group"
          >
            ...
            <p className="text-[9px] text-zinc-700 mt-2 font-mono tracking-widest">
              {new Date(item.created_at).toLocaleDateString("en-GB", {
                day: "2-digit",
                month: "numeric",
                year: "numeric",
          ...
          <AnimatePresence mode="wait">
            {loading && !report && (
              ...
              >
                <div className="inline-block relative mb-6">
                  <div className="w-10 h-10 border border-white/[0.05] border-t-red-900/50 rounded-full animate-spin" />
                </div>
                <p className="text-xs font-light tracking-[0.2em] text-zinc-500 uppercase">
                  Synthesizing Intelligence...
                </p>
              </motion.div>
            )}
            ...
            {report && (
              <motion.div
                ...
                className="bg-white/[0.01] border border-white/[0.03] backdrop-blur-3xl rounded-2xl sm:rounded-[2rem] p-6 sm:p-10 lg:p-16 shadow-[0_0_50px_rgba(0,0,0,0.5)]"
              >
                <div className="flex items-center gap-3 mb-8 pb-6 border-b border-white/[0.02]">
                  <BookOpen className="text-red-900/60 shrink-0" size={18} strokeWidth={1.5} />
